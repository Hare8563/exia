...
パックのアーカイブ形式は検討中。候補：
- **zip**: 汎用的、Rustクレートが豊富（`zip` crate）
- **XP3**: 吉里吉里ネイティブ形式、既存KiriKiriツールとの互換性あり

解凍処理はTauriバックエンド（Rust）側で行い、AppDataへ展開する。

---

## 未解決事項

- アーカイブ形式の最終決定（zip vs XP3）
- チェックサム検証の方式（MD5/SHA256）
- 部分更新（パック内の一部ファイルのみ差し替え）の対応有無
